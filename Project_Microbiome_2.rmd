...
``` 
We also made a heatmap of the  second dataset, genera.counts,to have a fist impression of the data:

Heatmap of genera.counts dataset:
```{r}
...
# Load the dataset
genera_counts <- read.delim("~/Downloads/genera.counts.tsv", header = TRUE, sep = "\t", row.names = 1)

# Summarize the data: calculate total counts for each genus
genera_sums <- colSums(genera_counts, na.rm = TRUE)

# Select the top 20 genera based on total counts
top_genera <- names(sort(genera_sums, decreasing = TRUE))[1:20]
filtered_data <- genera_counts[, top_genera]

# Normalize the data by scaling rows (samples)
normalized_data <- t(scale(t(filtered_data)))

# Create row and column labels
rownames(normalized_data) <- rownames(filtered_data)
colnames(normalized_data) <- top_genera

# Generate a heatmap
Heatmap(normalized_data,
        name = "Normalized Counts",           # Legend title
        col = colorRamp2(c(-2, 0, 2), c("blue", "white", "red")),  # Color gradient
        row_names_gp = gpar(fontsize = 8),    # Row font size
        column_names_gp = gpar(fontsize = 10), # Column font size
...
still doesn't look normally distributed. Doubled checked by conducting shapiro-wilk test again.
...
*Visualisation*
Next we visualised the difference in abundance of the bacteria (sum) btween the two groups (IBD and nonIBD)
...
Again the qq plots give the same resut as the Mardia's test and the histograms.
...
```
The p-values suggest significant differences between age groups 10-18 and 55-63. So next we will do a post hoc test, we will use Tukey's HSD. This test compares all group pairs, controls for family-wise error and it's straightforward to interpret and widely used for categorical variables in GLMs.
